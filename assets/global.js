...
const subSliderInit = (isUpdate, margin) => {
	if (
		...
		document.querySelectorAll(".js-media-sublist").forEach((elem, index) => {
			...
			const slidesPerView = slidesAmount < 6 ? slidesAmount : 6;
			let subSlider = new Swiper(elem, {
				...
				on: {
				  click: function(swiper, event) {
					...
					// 点击右侧边缘的缩略图
					if (clickedIndex === visibleEnd && clickedIndex < slidesLength - 1) {
					  // 点击第4个（索引3），显示第5个
					  swiper.slideTo(clickedIndex - 2); // 向前移动2个位置
					}
					// 点击左侧边缘的缩略图
					else if (clickedIndex === visibleStart && clickedIndex > 0) {
					  // 点击当前显示的第一个，显示前一个
					  swiper.slideTo(clickedIndex - 1);
					}
					// 正常情况让 slideToClickedSlide 处理
				  }
				},
				breakpoints: {
					1200: {
...
class VariantSelects extends HTMLElement {
	...
	updateVariantInput() {
		const productForms = document.querySelectorAll(
			`#product-form-${this.dataset.section}, #product-form-installment-${this.dataset.section}, .sticky-addtocart-btn-form`,
		);
	...
	updateQuickAddImage() {
		...
		const product_images = document.querySelectorAll(".js-addtocartBtn .product_image_item");
		if(product_images) {
	...
	toggleAddButton(disable = true, text, modifyClass = true) {
		...
		if (disable) {
			...
			const addButtonStrings = addButton.dataset.variantstringsAddtocart;
			addButtonText.textContent = addButtonStrings;
		}
